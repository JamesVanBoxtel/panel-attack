...
function select_screen.main()
  GAME.sceneDraw = select_screen.draw
  ...
  pages_amount = fill_map(template_map, map)
...
local function draw_button(x, y, w, h, str, halign, valign, no_rect)
  ...
  local render_x = x_padding + (y - 1) * 100 + spacing
  local render_y = y_padding + (x - 1) * 100 + spacing
  local button_width = w * 100 - 2 * spacing
  local button_height = h * 100 - 2 * spacing
  ...
  if x ~= 0 then
    ...
    if (select_screen.character_select_mode == "2p_net_vs" or select_screen.character_select_mode == "2p_local_vs") and cursor_data[2].state and cursor_data[2].state.cursor == str and ((str ~= "__Empty" and str ~= "__Reserved") or (cursor_data[2].position[1] == x and cursor_data[2].position[2] == y)) then
...
function select_screen.draw()
  if modalMessage then
    gprint(modalMessage, unpack(main_menu_screen_pos))
  else 
