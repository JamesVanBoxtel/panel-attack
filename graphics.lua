...
function Stack.drawAnalyticData(self, analytic, x, y)
  ...
  local gpmRounded = string.format("%0.1f", round(garbagePerMinute, 1))
  gprintf(gpmRounded .. "/m", x + iconToTextSpacing, y + 0, canvas_width, "left", nil, 1, fontIncrement)
  ...
  -- APS
  local actionsPerSecond = (analytic.data.swap_count + analytic.data.move_count) / (self.CLOCK / 60)
  local APSRounded = string.format("%0.1f", round(actionsPerSecond, 1))
  ...
  gprintf(APSRounded .. "/s", x + iconToTextSpacing, y + 0, canvas_width, "left", nil, 1, fontIncrement)
